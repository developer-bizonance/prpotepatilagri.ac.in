...
const getImageSrc = (url) => {
  if (!url) return "/assets/Domestic-tour/domestictour1.jpeg";
  if (url.startsWith("http://") || url.startsWith("https://")) return url;
  const cleanPath = url.startsWith("/") ? url : `/${url}`;
  return `${API_BASE}${cleanPath}`;
};

const DomesticTourGallery = () => {
  ...
  useEffect(() => {
    const fetchTours = async () => {
      try {
        setLoading(true);
        const res = await fetch(`${API_BASE}/api/eventGallery?category=Domestic Study Tour`);
        if (res.ok) {
          ...
          if (Array.isArray(data) && data.length > 0) {
            const formatted = data.map((item) => ({
              ...
              alt: item.title || 'Domestic Tour',
              year: item.year ? item.year.toString() : '2026',
              title: item.title || 'Domestic Tour',
            }));
    ...
    <>
      <div className="min-h-screen bg-gray-50 py-8 sm:py-18 px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-7xl mx-auto">
          <header className="mb-2 sm:mb-12">
            <h1 className="text-xl sm:text-4xl border-l-4 border-orange-400 pl-3 font-bold text-gray-900 mb-4">
              Domestic Study Tour
            </h1>
          </header>
          ...
          <div className="flex mb-5 sm:mb-12 overflow-x-auto">
            <div className="inline-flex rounded-lg border-gray-200 p-1">
              {tabs.map((tab) => (
                <button
                  key={tab}
                  onClick={() => setActiveTab(tab)}
                  className={`px-6 sm:px-8 py-2 text-xs m-1 font-semibold rounded-3xl transition-all duration-200 ${
                    activeTab === tab
...
export default DomesticTourGallery;