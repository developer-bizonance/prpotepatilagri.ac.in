...
import { Loader2, FileText, Calendar, FolderOpen, ExternalLink } from "lucide-react";

export default function AdmissionCategoryPage() {
  ...
  // Category Titles with added descriptions for a professional look
  const categoryTitles = {
    "course-layout": { 
      title: "Course Layout", 
      desc: "View the detailed layout and structure of our academic courses."
    },
    "eligibility": { 
      title: "Eligibility Criteria", 
      desc: "Check the required qualifications and criteria for admission."
    },
    "contact": { 
      title: "Admission Contact", 
      desc: "Get in touch with our admission helpdesk for queries and support."
    },
    "fee-structure": { 
      title: "Fee Structure", 
      desc: "Detailed breakdown of academic fees and payment schedules."
    },
    "fra-proposal": { 
      title: "FRA Proposal 2026-27", 
      desc: "Fee Regulating Authority (FRA) proposals and related official documents."
    },
  };

  const pageInfo = categoryTitles[category] || { 
    title: "Admission Document", 
    desc: "View related admission documents and resources."
  };
  ...
  const fetchDocuments = async () => {
    try {
      setIsLoading(true);
      const res = await fetch("http://localhost:4001/api/admissions");
      ...
      const filtered = data.filter((item) => (item.category || "course-layout") === category);
        ...
        {documents.length === 0 ? (
          <div className="text-center py-16 bg-white border border-dashed border-slate-300 rounded-2xl shadow-sm">
            ...
            <p className="text-slate-500 text-sm max-w-md mx-auto">
              Currently, there are no documents uploaded for {pageInfo.title}. Please check back later.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-5">
            {documents.map((doc) => {
              ...
              // Smart link logic (Supports both PDF and External Links if added in future)
              let finalLink = "#";
              let isLink = false;
